...
- **query/QueryEngine.java** — the RAG pipeline. Flow: resolve client scope → KNN search (TOP_K=40) → relevance-threshold + relative-distance filter → lexical focus (stemmed, filename-aware) → diversify per file (`MAX_CHUNKS_PER_FILE=4`, `MAX_CONTEXT_CHUNKS`) → template filter → **kind-file injection** (question names a doc kind → that type's files get guaranteed seats, post-prune) → LLM answer (stream/non-stream) with **today's date + extracted-obligations context** (action-flavored questions get the deadlines/timeline rows as data — retrieval alone missed due dates) → `groupMatchesByFile` + **`trimSourcesToCited`** (chips = files named in answer; **empty for clarifying questions** via `isClarifyingQuestion`). Analytics/inventory paths enforce the classified subject's qualifier IN CODE (`subjectQualifiers`/`fileMatchesQualifiers` — "Meridian GSTR-3B" can never sum Zenlite's return); LIST intent with a per-item detail ask ("…and by when?") overrides to LOOKUP. `detectFileScope` honors a bare filename (or pasted absolute path) that resolves to exactly one in-scope file — `answerFromFileScope` then reads that file's chunks **once** and answers from them directly (reusing the same capped list for the source chips), bypassing semantic search.
...
Dormant unless clients registered. `ClientResolver` (NONE|SCOPED|CLARIFY — never guesses; a question naming NO client is a GENERAL question answered across everything, even with one client registered; CLARIFY only on genuine ambiguity, i.e. a word/alias matching 2+ clients), `ClientMatcher` (match client identifiers in question; a shared distinctive word like "Verma" for two Vermas returns ALL its owners so the clarify lists exactly those), `MembershipEngine` (auto-tag a file to a client only on EXACTLY ONE identifier match; else unassigned), `EntitySuggester` (detect candidate clients from docs → suggestions; entities with no human-readable name — bare GSTIN/PAN — are never suggested). Scope tokens: `__all__` (unscoped), `__unassigned__` (personal). Clarify chips offer the matched clients + "All documents".
